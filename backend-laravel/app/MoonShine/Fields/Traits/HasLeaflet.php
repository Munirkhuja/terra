...
namespace App\MoonShine\Fields\Traits;
...
trait HasLeaflet
{
    ...
    public function LatitudeAttributes(array $attributes): static
    {
        $this->latitudeAttributes = $this->getAttributes()
            ...
            ->when(
                ...
                fn(ComponentAttributesBagContract $attributes) => $attributes->merge(
                    $this->latitudeAttributes->getAttributes()
                )
            );
    ...
    public function longitudeAttributes(array $attributes): static
    {
        $this->longitudeAttributes = $this->getAttributes()
            ...
            ->when(
                ...
                fn(ComponentAttributesBagContract $attributes) => $attributes->merge(
                    $this->longitudeAttributes->getAttributes()
                )
            );
    ...
    {
        return [
            $this->getLatitudeField() => $data[$this->getLatitudeField()] ?? data_get(
                    $this->getDefault(),
                    $this->getLatitudeField(),
                    $this->min
                ),
            $this->getLongitudeField() => $data[$this->getLongitudeField()] ?? data_get(
                    $this->getDefault(),
    ...
    protected function resolvePreview(): Renderable|string
    {
        ...
        if (!$value) {
            ...
            $value = [
                $this->getLatitudeField() => data_get($item, $this->getLatitudeField()),
                $this->getLongitudeField() => data_get($item, $this->getLongitudeField()),
            ];
        }


        $latitude = $value[$this->getLatitudeField()] ?? '0';
        $longitude = $value[$this->getLongitudeField()] ?? '0';

        $url = "https://yandex.ru/maps/?pt={$longitude},{$latitude}&z=15&l=map";
